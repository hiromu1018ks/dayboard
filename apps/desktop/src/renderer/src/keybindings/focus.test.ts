 ...
 *
 * focusSection / getFocusedSection / focusAdjacentSection / focusItemInCurrentSection の検証。
...
describe('focusSection / getFocusedSection（[§3.2]）', () => {
  beforeEach(() => {
    setupSections([
      { section: 'theme' },
  ...
  it('focusSection で指定セクションへフォーカス', () => {
    expect(focusSection('todo')).toBe(true);
    expect(getFocusedSection()).toBe('todo');
  });

  it('focusSection で reflection へフォーカス', () => {
    expect(focusSection('reflection')).toBe(true);
    expect(getFocusedSection()).toBe('reflection');
  });

  it('存在しないセクションは false', () => {
    document.body.innerHTML = '';
    expect(focusSection('todo')).toBe(false);
  });

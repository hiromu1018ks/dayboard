...
- テスト分层:
  ...
  - Integration: `pnpm test:integration`（**直列実行必須**、SQLite ファイル使用）
...
- データストアは SQLite（`@libsql/client` / `libsql`）。外部 DB エンジンのインストール・起動は不要。
- `DATABASE_URL` は未設定で動く（実アプリは `userData/dayborad.db`、ローカル開発の `pnpm dev:api` 単独起動等では CWD 相対の `dayborad.db` を使用）。明示的に場所を指定したい場合のみ `.env` で設定。
- Integration テストは `DATABASE_URL`（例: `file:./dayborad_test.db`）で指定した SQLite ファイルへマイグレーション済みが前提。CI は `.github/workflows/ci.yml` 参照。E2E は起動ごとに一時 userData ディレクトリ内の `dayborad.db` を使うため事前準備不要。
...
- **Renderer は永続化を直接操作しない**。すべて Hono API（`fetch`）経由。`apps/desktop/src/renderer` 配下で `@libsql/client` / `libsql` / `repository` を直接 import しない。
- **Main プロセスはドメインロジックを持たない**。起動・ライフサイクル（SQLite ファイルパス解決 → DB 接続確認 → migration → Hono localhost 起動 → Renderer へ API URL 注入・flush 制御）のみ。

...
本書は、[要件定義書 10. データ設計案](dayborad_requirements.md) のTypeScript型案を、PostgreSQLの物理スキーマ（テーブル・制約・インデックス・マイグレーション方針）に落とし込んだ設計契約である。実装者がDDLを書く際の単一の真実源とする。
...
TypeScript型（`TodoItem.dayNoteId`）↔ PostgreSQLカラム（`todo_items.day_note_id`）の変換は、リポジトリ層で一括して行う（[architecture.md §4](architecture.md)）。
...
## 10. PostgreSQL設定ノート（MVPローカル運用）

- エンコーディング: `UTF8`、ロケール: `C` または `ja_JP.UTF-8`。
- 接続プール: [architecture.md](architecture.md) のリポジトリ層でプール管理（推奨: `pg` ライブラリの `Pool`、`max: 5` 程度）。
- タイムゾーン: DBの `timezone` 設定には依存せず、`timestamptz` はUTCで保存しアプリ層でローカル変換（[§8](#8-日付の扱い)）。
...
各メソッドはドメイン型（要件 10 のTS型）を返し、PostgreSQLの `snake_case` 表現との変換を内部で吸収する。

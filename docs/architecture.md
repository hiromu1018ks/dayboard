...
| C5 | PostgreSQLを主データストアとする | 13.1, 13.2 |
...
### 2.1 決定：Electronデスクトップアプリ + ローカルHonoサーバー + ローカルPostgreSQL
...
- ショートカットの制御、ローカルプロセス管理、PostgreSQL起動管理のいずれもNode層で完結する。
...
**なぜローカルPostgreSQLか**
- C5がPostgreSQLを主データストアに指定しているため。
- ローカルファースト・オフライン（C6）を満たすため、データは端末ローカルに置く。
- 単一ユーザー（C7）のため、1つのローカルデータベースインスタンスで完結する。

> **DB同梱に関する注記:** Electronアプリ内にPostgreSQLサーバープロセスを同梠・起動管理する方式は、バイナリサイズと起動処理の複雑さを増す。本書では要件定義のPostgreSQL指定（C5）を尊重してPostgreSQLを採用するが、データアクセスは [database_schema.md](database_schema.md) で定義するリポジトリインターフェース経由に限定する。これにより、配布時にPostgreSQL同梠が過剰と判断された場合は、リポジトリ実装をSQLite等に差し替え可能な構造を保つ。
...
| PostgreSQL起動・接続 | ローカルPostgreSQLインスタンスの起動確認・接続プール管理・マイグレーション実行 |
...
└──────────────────┬───────────────────────────┘
                   │ interface
┌──────────────────▼───────────────────────────┐
│  Repository Layer (interface + pg impl)      │
...
│   └── repository/         # リポジトリinterface + PostgreSQL実装 + migration
├── apps/
...
```text
1. Electron main 起動
2. PostgreSQL 接続確認（未起動なら起動待ち/起動）
...
- 保存状態（`idle`/`saving`/`saved`/`error`）をUI右上に表示し、失敗時はリトライする。
...
| データストア | ローカルPostgreSQL（リポジトリ経由） | C5。リポジトリIFで差し替え可能性を保持 |

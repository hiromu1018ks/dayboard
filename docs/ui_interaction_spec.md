...
> **入力要素フォーカス中のスルー（重要）:** Normal状態でも、入力要素（`<input>` / `<textarea>` / contenteditable = CodeMirror の `.cm-content`）へフォーカスしている間は、**Vim コマンド（hjkl/x/dd/u 等）を一切処理せず文字入力へ貫通** する。ユーザーが「TODO追加入力欄へ普通にフォーカスして `k` を入力」するとき、`k` が「上へ移動」として奪われないようにするため。カード選択（`button[data-focus-item]` にフォーカス）中のみ Vim コマンドが有効。明示的に Insert へ入りたい場合は `i` を使う（選択中カードの本文編集モードへ）。
...
**Vimキーバインド:**
1. Normal状態で追加入力欄（番哨行）を選択中に `i`/`Enter` → Insert状態で追加入力欄へ
2. 入力後 `Enter` で確定 → **Normal状態へ戻り**、選択要素（追加入力欄）へフォーカスを戻す（hjkl が即座に使える）。連続追加する場合は再度 `i`/`Enter`、または `o`/`O` を使う
3. 空入力で `Enter` → Normal状態へ戻り追加入力欄から抜ける
4. `Esc` でも Insert→Normal、追加入力欄からフォーカスを外しTODOリストへ

### 5.2 TODO本文編集の確定

- ダブルクリック、またはフォーカス中に `Enter`（標準）/ `i`/`Enter`/`a`（Vim Normal）で編集モードへ
  - Vim: 選択中の TODO/Blocker アイテムで `i`/`Enter`/`a` を押すと、当該アイテムの編集モード（input 表示）へ入り Insert 状態へ遷移する
- 編集確定は `Enter`（標準/Vim ともに）→ 変更があれば保存し編集モードを終了。Vim は同時に **Normal状態へ戻り**、選択要素（button）へフォーカスを戻す（hjkl が即座に使える）
- 編集キャンセルは `Esc` → 編集モードを終了。Vim は同時に Normal状態へ戻る
...
3. モーダル open（設定 / キーバインドガイド） → 開いているモーダルを閉じるのみ
...
## 10.5 キーバインドガイド（要件 8.1、AC-23）

キーバインドがわからないユーザーがいつでもショートカットを確認できるガイドを提供する。

### 起動方法

| トリガー | 動作 |
|----------|------|
| `?` キー（修飾キーなし） | ガイドをトグル（閉じているとき開く / 開いているとき閉じる） |
| ヘッダーのヘルプアイコン | ガイドを開く（仕事整理モードは `Header` の歯車の横、ノートモードは `NoteMode` の「戻る」案内の横に配置） |

### `?` キーの入力要素貫通仕様

`?` は標準キーバインド・Vimキーバインドのどちらでも有効だが、**テキスト入力欄にフォーカス中は文字入力として扱いガイドを開かない**。テキスト入力欄とは `vim.ts` の `isTextInputElement` と同基準（`<input>` / `<textarea>` / `contenteditable` = CodeMirror の `.cm-content`）。入力欄以外（カード・`section`・ボタン等）にフォーカスがある状態、またはフォーカス外のときのみ開く。

### 表示内容のモード別切替

ガイドは現在の表示モード（仕事整理 / ノート）とキーバインドモード（標準 / Vim）に応じて表示セクションを切り替える。不要な情報を省くことで「今すぐ使えるキー」だけを提示する。

| 状態 | 表示セクション |
|------|----------------|
| 共通（全状態） | 基本ショートカット（`⌘J` `⌘T` `Option←/→` `⌘\`）+ ガイド起動（`?`） |
| 仕事整理 + 標準 | 仕事整理モード（`⌘1/2/3` `⌘Enter`、[§11.2]） |
| 仕事整理 + Vim | 仕事整理モードの Vim 操作（`h/j/k/l` `gg/G` `i/a/o/x/dd/u/Ctrl+r` `Space n/1/2/3`、[§11.4]） |
| ノート + 標準 | ノートモード（`⌘Enter` TODO化 `⌘Shift+B` 障害化 `⌘Shift+M` 時刻見出し、[§11.3]） |
| ノート + Vim | ノートモード（[§11.3]）+ Vim（CodeMirror 委譲、`Space t/b`、[§11.4]） |

### 閉じ方

`Esc`（[§9.2] 段3）/ 背景クリック / 閉じるボタン / もう一度 `?`。

ガイドモーダルは `SettingsModal` と同一パターン（`fixed inset-0 z-50 bg-black/40` + `role="dialog"` + `aria-modal="true"` + 背景クリックで閉じる）とする。
